...
void sinc(const dvec *restrict input, dvec *restrict output) {
	for(double *o = output->vec, *i = input->vec; o < output->vec + output->size; ++i, ++o)
		*o = d_equal(*i, .0) ? 1. : sin(*i) / (*i + EPSILON);
}

dvec *sinc_(const dvec *restrict vec) {
	dvec *rt = dvec_init(vec->size, false);
	for(double *r = rt->vec, *i = vec->vec; r < rt->vec + rt->size; ++i, ++r)
		*r = d_equal(*i, .0) ? 0 : cos(*i) / (*i + EPSILON) - sin(*i) / (pow(*i, 2.) + EPSILON);
	return rt;
}
