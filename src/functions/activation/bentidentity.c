...
void bentidentity(const dvec *restrict input, dvec *restrict output) {
	for(double *o = output->vec, *i = input->vec; o < output->vec + output->size; ++i, ++o)
		*o = (sqrt(pow(*i, 2.) + 1.) - 1.) / 2. + *i;
}

dvec *bentidentity_(const dvec *restrict vec) {
	dvec *rt = dvec_init(vec->size, false);
	for(double *r = rt->vec, *i = vec->vec; r < rt->vec + rt->size; ++i, ++r)
		*r = *i / (2. * sqrt(pow(*i, 2.) + 1.) + EPSILON) + 1.;
	return rt;
}
