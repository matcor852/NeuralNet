...
static const double elu_alpha = 1.;

void elu(const dvec *restrict input, dvec *restrict output) {
	for(double *o = output->vec, *i = input->vec; o < output->vec + output->size; ++i, ++o)
		*o = *i > .0 ? *i : elu_alpha * (expn(*i) - 1.);
}

dvec *elu_(const dvec *restrict vec) {
	dvec *rt = dvec_init(vec->size, false);
	for(double *r = rt->vec, *i = vec->vec; r < rt->vec + rt->size; ++i, ++r)
		*r = *i < .0 ? (*i > .0 ? *i : elu_alpha * (expn(*i) - 1.) + elu_alpha) : 1.;
	return rt;
}
