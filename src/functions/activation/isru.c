...
static const double isru_alpha = .5;

void isru(const dvec *restrict input, dvec *restrict output) {
	for(double *o = output->vec, *i = input->vec; o < output->vec + output->size; ++i, ++o)
		*o = *i / (sqrt(1. + isru_alpha * pow(*i, 2)) + EPSILON);
}

dvec *isru_(const dvec *restrict vec) {
	dvec *rt = dvec_init(vec->size, false);
	for(double *r = rt->vec, *i = vec->vec; r < rt->vec + rt->size; ++i, ++r)
		*r = pow(1. / sqrt(1. + isru_alpha * pow(*i, 2.)), 3.);
	return rt;
}
