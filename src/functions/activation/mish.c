...
void mish(const dvec *restrict input, dvec *restrict output) {
	for(double *o = output->vec, *i = input->vec; o < output->vec + output->size; ++i, ++o)
		*o = *i * tanh(log(1. + expn(*i)));
}

dvec *mish_(const dvec *restrict vec) {
	dvec *rt = dvec_init(vec->size, false);
	for(double *r = rt->vec, *i = vec->vec; r < rt->vec + rt->size; ++i, ++r) {
		const double e = expn(*i), e2 = expn(*i * 2.);
		*r = e * (4. * (*i + 1.) + 4. * e2 + expn(*i * 3.) + e * (*i * 4. + 6.)) / (pow(2 * e + e2 + 2., 2.) + EPSILON);
	}
	return rt;
}
