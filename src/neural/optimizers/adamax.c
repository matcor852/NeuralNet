...
double adamax_wgd(optimizer *restrict o, const double gradient) {
	*(o->mwt) = b1 * *(o->mwt) + c1 * gradient;
	*(o->vwt) = fmax(b2 * *(o->vwt), fabs(gradient));
	const double m_rect = *(o->mwt) / (1. - o->b1t);

	double rt = m_rect * o->l_rate / *(o->vwt);
	o->mwt++;
	o->vwt++;
	return rt;
}

double adamax_bgd(optimizer *restrict o, const double gradient) {
	*(o->mbt) = b1 * *(o->mbt) + c1 * gradient;
	*(o->vbt) = fmax(b2 * *(o->vbt), fabs(gradient));
	const double m_rect = *(o->mbt) / (1. - o->b1t);

	double rt = m_rect * o->l_rate / *(o->vbt);
	o->mbt++;
	o->vbt++;
	return rt;
}

void adamax_next_layer(optimizer *restrict o) {
	o->curr_layer--;
...
void adamax_next_iter(optimizer *restrict o) {
	o->iter++;
	o->b1t = pow(b1, o->iter);
	o->curr_layer = o->nb - 1;
	o->mwt = o->Mwt[o->curr_layer]->vec;
