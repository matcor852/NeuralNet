...
double nadam_wgd(optimizer *restrict o, const double gradient) {
	...
	*(o->vwt) = b2 * *(o->vwt) + c2 * pow(gradient, 2);
	...
	double rt = o->l_rate / (sqrt(v_rect) + EPSILON) * (b1 * m_rect + (1. - b1) * gradient / (1. - o->b1t));
	o->mwt++;
	o->vwt++;
	return rt;
}

double nadam_bgd(optimizer *restrict o, const double gradient) {
...
void nadam_next_layer(optimizer *restrict o) {
	o->curr_layer--;
...
void nadam_next_iter(optimizer *restrict o) {
	o->iter++;
	o->b1t = pow(b1, o->iter);
	o->b2t = pow(b2, o->iter);
	o->curr_layer = o->nb - 1;
